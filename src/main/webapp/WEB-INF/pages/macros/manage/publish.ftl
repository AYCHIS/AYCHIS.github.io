...
<form action='publish.do' method='post'>
  ...
  <#if missingMetadata>
    <@s.submit id="publishButton" cssClass="button btn btn-default" name="publish" key="button.publish" disabled="true"/>
  ...
  <#elseif resource.lastPublished?? && resource.identifierStatus == "UNRESERVED" && resource.status != "REGISTERED">
    <@s.submit cssClass="confirmPublishMinorVersion" cssClass="button btn btn-default" id="publishButton" name="publish" key="button.publish"/>
    ...
    <#elseif resource.identifierStatus == "PUBLIC_PENDING_PUBLICATION">
        ...
        <#if !resource.isAlreadyAssignedDoi() && resource.status == "PRIVATE">
          ...
          <#if !resource.lastPublished??>
            <@s.submit cssClass="confirmPublishMajorVersionWithoutDOI btn btn-default" id="publishButton" name="publish" key="button.publish"/>
            <i class="infoImg fa fa-exclamation-triangle"> </i>
          <!-- and the resource has been published before, the next publication is a new minor version -->
          <#else>
            <@s.submit cssClass="confirmPublishMinorVersion btn btn-default" id="publishButton" name="publish" key="button.publish"/>
            <i class="infoImg fa fa-exclamation-triangle"> </i>
          </#if>
          <div class="info autop">
        ...
        <#elseif resource.status == "PUBLIC" || resource.status == "REGISTERED">
          <@s.submit cssClass="confirmPublishMajorVersion btn btn-default" id="publishButton" name="publish" key="button.publish"/>
